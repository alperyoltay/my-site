...
class tiktok:
    ...
    def update_title(self):
        ...
        ctypes.windll.kernel32.SetConsoleTitleW(
            f"TikTok Username Checker | Available: {self.available} | Unavailable: {self.unavailable} | Checked: {(self.available + self.unavailable)} | Remaining: {remaining}"
        )
    ...
    def check_username(self, username):
        ...
        with requests.Session() as session:
            ...
            if r.status_code == 200:
                self.unavailable += 1
                self.print_console("Kullanım dışı", username)
                with open("kullanilmayan.txt", "a") as f:
                        f.write(username + "\n")
            elif r.status_code == 404:
                self.available += 1
                self.print_console("Mevcut veya Yasaklı", username, Fore.GREEN)
    ...
    def load_usernames(self):
        if not os.path.exists("usernames.txt"):
            self.print_console("Console", "File usernames.txt bulunamadı")
            time.sleep(10)
            os._exit(0)
        with open("usernames.txt", "r", encoding = "UTF-8") as f:
            ...
            if not len(self.usernames):
                self.print_console("Console", "Kullanıcı adı yüklenmedi usernames.txt")
                time.sleep(10)
                os._exit(0)
    ...
    def main(self):
        os.system(clear)
        if clear == "cls":
            ctypes.windll.kernel32.SetConsoleTitleW("TikTok Username Checker")
